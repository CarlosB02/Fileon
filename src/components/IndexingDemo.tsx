...
const IndexingDemo = ({ embedded = false }: { embedded?: boolean }) => {
            ...
            <div className="flex-1 p-6 bg-slate-50/50 relative">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                    <AnimatePresence mode='popLayout'>
                        ...
                        {(() => {
                            const displayDocs = (isMobile && filteredDocs.length > 3)
                                ? filteredDocs.slice(0, 3)
                                : filteredDocs;

                            const remainingCount = filteredDocs.length - 3;
                            ...
                            return (
                                <>
                                    ...
                                    {isMobile && filteredDocs.length > 3 && (
                                        <motion.div
                                            layout
                                            initial={{ opacity: 0, scale: 0.8 }}
                                            animate={{ opacity: 1, scale: 1 }}
                                            exit={{ opacity: 0, scale: 0.8 }}
                                            transition={{ duration: 0.2 }}
                                            className="bg-blue-50 p-3 rounded-lg border border-blue-200 hover:bg-blue-100 transition-all cursor-pointer flex flex-col items-center justify-center text-center h-full min-h-[120px]"
                                            onClick={() => {
                                                // Optional: Maybe scroll to a full list or just expand? 
                                                // For now, let's just show it acts as a "more" indicator
                                                const el = document.getElementById('indexdemo');
                                                if (el) el.scrollIntoView({ behavior: 'smooth' });
                                            }}
                                        >
                                            <div className="w-10 h-10 bg-white rounded-full flex items-center justify-center mb-2 shadow-sm text-blue-600 font-bold">
                                                +{remainingCount}
                                            </div>
                                            <p className="text-xs font-bold text-blue-700">Ver mais documentos</p>
                                            <p className="text-[10px] text-blue-500 mt-1">Existem mais resultados</p>
                                        </motion.div>
                                    )}
                                </>
                            );
    ...
    return (
        <section id="indexdemo" className="py-12 bg-white relative overflow-hidden">
